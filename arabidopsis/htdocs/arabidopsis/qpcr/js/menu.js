...
{ 
  ...
  //a custom change to point link to NCBI Taxonomy Browser
  this.hreference = null;
  //if (this.desc == "Other Plants [13]"){
  if (this.desc.match(/Others/)){

	this.hreference = "#";
  }
  else if (this.desc.match(/Other Angiosperms/)){

	this.hreference = "#";
  }

  else {
  	this.hreference = "http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?mode=Info&id="+hreference;
  }
  this.id = -1;
  this.navObj = 0;
  this.iconImg = 0; 
	...
	if (USEFRAMES)
	  docW = docW + "<a href='" + this.hreference + "' TARGET=\"basefrm\" "
		...
		if (this.hreference == "#"){ 
			...
			docW = docW + "<a href='" + this.hreference + "' TARGET=_blank ";
		}
    	//docW = docW + "<a href='" NCBI_taxonomy_link + this.hreference + "' TARGET=_blank "
    if (isTextLink) {
...
var PRESERVESTATE = 0;
var ICONPATH = 'http://plantta.tigr.org/imgs/treemenu/';
var HIGHLIGHT = 0;
...
function generateCheckBox(itemLabel, checkBoxDOMId) {
    var newObj;
    newObj = gFld(itemLabel, checkBoxDOMId);
    newObj.prependHTML = '<td valign=middle><input type=checkbox name="taxon_id" value="'+checkBoxDOMId+'" id='+checkBoxDOMId+' ></td>';
    newObj.xID = checkBoxDOMId;
    return newObj;
}

function generateCheckBoxNode(itemLabel, checkBoxDOMId){
	//using this function for the node prevents the taxon ids from being submitted
    var newObj;
    newObj = gFld(itemLabel, checkBoxDOMId);
    newObj.prependHTML = '<td valign=middle><input type=checkbox id='+checkBoxDOMId+' onclick="clickMe('+checkBoxDOMId+')" ></td>';
    newObj.xID = checkBoxDOMId;
    return newObj;
}


function clickMe(id, mode){
	//recursivley checks the boxes of the children of the node
    var node = findObj(id)
    if( node == null){
        return;
    }
    if(mode == null){
        mode = getElById(node.xID).checked;
    }
    getElById(node.xID).checked = mode;
    var childNum = node.nChildren;
    for (var i = 0; i< childNum;i++){
        clickMe(node.children[i].xID, mode);
    }
}
